...
if not os.path.exists(PUBLIC_KEY_FILE) or not os.path.exists(PRIVATE_KEY_FILE):
    #first run create RSA keys protexted with the master password
...
def removePassword():
    ...
    try:
        ...
        if 1 <= num <= len(webs):  #check that choice is valid
            ...
            removed = passwordData.pop(selectedSite)  #rmove it from dictionary
...
def updatePassword():
    ...
    try:
        ...
        if 1 <= num <= len(webs):
            ...
            #confrim the decryption right away
